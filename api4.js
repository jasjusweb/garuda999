if(typeof QRCode==='undefined'){let s=document.createElement('script');s.src='https://cdn.jsdelivr.net/npm/qrcodejs@1.0.0/qrcode.min.js';s.async=true;document.head.appendChild(s)}
function waitForQRCode(timeoutMs){return new Promise(function(resolve,reject){if(typeof QRCode!=='undefined'){resolve();return}var start=Date.now();var t=setInterval(function(){if(typeof QRCode!=='undefined'){clearInterval(t);resolve()}else if(Date.now()-start>(timeoutMs||8000)){clearInterval(t);reject(new Error('Library QRCode gagal dimuat.'))}},50)})}
function ensureQrisFontAwesome(){
if(document.querySelector('link[data-qris-fa="1"]'))return;
let l=document.createElement('link');
l.rel='stylesheet';
l.setAttribute('data-qris-fa','1');
l.href='https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.7.1/css/all.min.css';
document.head.appendChild(l);
}
function isMobileDevice(){return/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)}
function escMerchantLabel(s){return String(s??'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/"/g,'&quot;')}
function parseQrisMerchantName(q){if(!q)return null;function w(pl,d){if(!pl||d>8)return null;let i=0;while(i+4<=pl.length){const tag=pl.substring(i,i+2);const len=parseInt(pl.substring(i+2,i+4),10);if(Number.isNaN(len)||len<0||i+4+len>pl.length)return null;const val=pl.substring(i+4,i+4+len);if(tag==='59')return val.trim();const tn=parseInt(tag,10);if(!Number.isNaN(tn)&&tn>=26&&tn<=51){const inner=w(val,d+1);if(inner!=null&&inner!=='')return inner}i+=4+len}return null}let s=q.trim();if(s.length>8&&s.slice(-8,-4)==='6304')s=s.slice(0,-8);return w(s,0)}
...
function bootQrisCepatWhenReady(){
  const run=function(){
    ...
    jQuery(function($){startQrisCepatApp($)});
  };
  if(typeof jQuery!=='undefined'){run();return}
  let tries=0;
  const timer=setInterval(function(){
    tries++;
    if(typeof jQuery!=='undefined'){clearInterval(timer);run()}
    else if(tries>200){clearInterval(timer);console.error('[QRIS] jQuery not found; deposit tabs skipped')}
  },50);
}
function startQrisCepatApp($){
if(window.__qrisCepatV11HandlersBound){
  if(typeof window.__qrisCepatInjectForms==='function')window.__qrisCepatInjectForms($);
  return;
}
window.__qrisCepatV11HandlersBound=true;
window.qrisDepoTabsVisible=true;window.qris2TabEnabled=true;let qT=null;let bC=null;let pIR=null;let accountData=null;function pad2(n){return String(n).padStart(2,'0')}function getTodayHistoryQuery(){const now=new Date();const s=new Date(now.getFullYear(),now.getMonth(),now.getDate(),0,0,0);const e=new Date(now.getFullYear(),now.getMonth(),now.getDate(),23,59,0);return{searchDateFrom:pad2(s.getDate())+'-'+pad2(s.getMonth()+1)+'-'+s.getFullYear()+' '+pad2(s.getHours())+':'+pad2(s.getMinutes()),searchDateTo:pad2(e.getDate())+'-'+pad2(e.getMonth()+1)+'-'+e.getFullYear()+' '+pad2(e.getHours())+':'+pad2(e.getMinutes())}}function transMatchesQrDeposit(hit,qAmount){if(!hit||hit.transType!=='Deposit')return false;return Math.abs(Number(hit.amount)-Number(qAmount))<0.00001}function isDepositPending(hit){if(!hit)return false;const si=Number(hit.statusInt);if(si===10)return true;return/^pending$/i.test(String(hit.status||'').trim())}function isDepositRejected(hit){if(!hit)return false;const si=Number(hit.statusInt);if(si===30)return true;const st=String(hit.status||'').toLowerCase();return/\b(cancel|reject|tolak|gagal|failed)\b/.test(st)}function pickLatestByTransId(rows){if(!rows||!rows.length)return null;return rows.reduce((a,b)=>Number(a.transId)>Number(b.transId)?a:b)}function findPendingForAmount(list,amt){const p=(list||[]).filter(t=>t.transType==='Deposit'&&transMatchesQrDeposit(t,amt)&&isDepositPending(t));return pickLatestByTransId(p)}function findRejectedForActiveQr(list,qD){const c=(list||[]).filter(t=>t.transType==='Deposit'&&transMatchesQrDeposit(t,qD.amount)&&isDepositRejected(t));if(!c.length)return null;if(qD.transId!=null&&qD.transId!==''){const tid=String(qD.transId);const by=c.find(t=>String(t.transId)===tid);if(by)return by}return pickLatestByTransId(c)}function isDepositApproved(hit){if(!hit)return false;const si=Number(hit.statusInt);if(si===15)return true;const st=String(hit.status||'').toLowerCase().trim();return st==='approved'||st.startsWith('approve')}function findApprovedForActiveQr(list,qD){const c=(list||[]).filter(t=>t.transType==='Deposit'&&transMatchesQrDeposit(t,qD.amount)&&isDepositApproved(t));if(!c.length)return null;if(qD.transId!=null&&qD.transId!==''){const tid=String(qD.transId);const by=c.find(t=>String(t.transId)===tid);if(by)return by}return pickLatestByTransId(c)}function compressImage(file,maxSizeInMB=1){return new Promise((resolve,reject)=>{const reader=new FileReader();reader.readAsDataURL(file);reader.onload=(event)=>{const img=new Image();img.src=event.target.result;img.onload=()=>{const canvas=document.createElement('canvas');const ctx=canvas.getContext('2d');const MAX_WIDTH=1920;const MAX_HEIGHT=1080;let width=img.width;let height=img.height;if(width>height){if(width>MAX_WIDTH){height*=MAX_WIDTH/width;width=MAX_WIDTH}}else{if(height>MAX_HEIGHT){width*=MAX_HEIGHT/height;height=MAX_HEIGHT}}
...
<div class="qris-cepat-wrapper-v11"><div class="depo-tabs"><button type="button" class="tab" data-target="manual"><i class="fa-solid fa-keyboard"></i> Manual</button><button type="button" class="tab" data-target="qris"><i class="fa-solid fa-qrcode"></i> QRIS</button><button type="button" class="tab" data-target="qris2"><i class="fa-solid fa-credit-card"></i> QRIS 2</button></div><div class="cepat-form-container qris-form-container"><div class="cepat-input-area"><div class="qris-logo-container"><img src="https://upload.wikimedia.org/wikipedia/commons/thumb/e/e1/QRIS_logo.svg/500px-QRIS_logo.svg.png" alt="QRIS Logo" class="qris-logo"></div><div class="form-group"><label>Pilih Nominal Cepat</label><div class="quick-amount-selector">${quickAmountButtonsHTML}</div></div><div class="form-group"><label>Atau, Masukkan Jumlah Lain</label><div class="input-group-promo"><span class="input-group-text">Rp</span><input type="text" class="form-control qris-amount-input" data-type="qris" placeholder="Contoh: 50.000" autocomplete="off"><span class="clear-input-btn">&times;</span></div><div class="unique-code-info"></div><div class="qris-validation-message"></div></div><div class="form-group promo-dropdown-container"><label>Promosi</label><select class="form-control promo-select"><option value="">Memuat promosi...</option></select><div class="selected-promo-info"></div></div><button type="button" class="cepat-button qris-button"><span class="btn-text"><i class="fa-solid fa-qrcode"></i> Buat QRIS</span></button></div><div class="result-container"></div></div><div class="cepat-form-container qris2-form-container"><div class="cepat-input-area"><div class="form-group"><div class="qris2-barcode-container"><i class="fa-solid fa-spinner fa-spin"></i>&nbsp; Memuat barcode... </div></div><div class="form-group"><label>Nominal Yang Ditransfer</label><div class="input-group-promo"><span class="input-group-text">Rp</span><input type="text" class="form-control qris2-amount-input" placeholder="Contoh: 50.000" autocomplete="off"></div><div class="qris-validation-message"></div></div><div class="form-group"><label>Upload Bukti Transfer (Wajib)</label><input type="file" class="form-control qris2-proof-input" accept="image/png,image/jpeg,image/jpg,application/pdf,.pdf" name="receipt"><div class="qris-validation-message"></div></div><button type="button" class="cepat-button qris2-button"><span class="btn-text"><i class="fa-solid fa-paper-plane"></i> Konfirmasi Pembayaran</span></button></div><div class="result-container"></div></div></div>
`;
async function loadPromotions(formContext){const promoSelect=formContext.find('.promo-select');const promoContainer=formContext.find('.promo-dropdown-container');promoContainer.show();try{const dcr=await fetch('https://qriscepat.jasjusweb.workers.dev/api/deposit-config');if(!dcr.ok){throw new Error('Gagal mengambil konfigurasi deposit.')}
const dc=await dcr.json();
const _wCfg=promoSelect.closest('.qris-cepat-wrapper-v11');
window.qrisDepoTabsVisible=dc.showDepoTabs!==false;window.qris2TabEnabled=dc.qris2Enabled!==false;
if(_wCfg.length){if(window.qrisDepoTabsVisible===false){_wCfg.find('.depo-tabs').hide()}else{_wCfg.find('.depo-tabs').show()}if(window.qris2TabEnabled===false){const tQ2=_wCfg.find('.tab[data-target=qris2]');tQ2.hide();if(tQ2.hasClass('active')){_wCfg.find('.tab[data-target=manual]').trigger('click')}_wCfg.find('.qris2-form-container').hide()}else{_wCfg.find('.tab[data-target=qris2]').show()}}
if(!dc.success){
    promoSelect.empty();
...
if(!aQD)return;
try{const qD=JSON.parse(aQD);if(!qD||!qD.expireTime||Date.now()>=qD.expireTime)return;if(!(qD.qrisString&&typeof qD.initialBalance==='number'))return;}catch(e){return}
const qW=$('.qris-cepat-wrapper-v11:visible .qris-form-container').first();
...
window.__qrisCepatInjectForms=injectQrisDepositForms;
function observeQrisDepositForms($){
if(window.__qrisCepatV11Observer)return;
if(typeof MutationObserver==='undefined'){setInterval(function(){injectQrisDepositForms($)},1500);return}
let scheduled=null;
window.__qrisCepatV11Observer=new MutationObserver(function(){
if(scheduled)return;
scheduled=setTimeout(function(){scheduled=null;injectQrisDepositForms($)},100);
});
window.__qrisCepatV11Observer.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['action','class','style']});
// Fallback poll for first ~20s in case SPA sets action late without useful mutations
let polls=0;const pollTimer=setInterval(function(){injectQrisDepositForms($);polls++;if(polls>=20)clearInterval(pollTimer)},1000);
}
injectQrisDepositForms($);
observeQrisDepositForms($);

const sDS=(w,dA)=>{if(bC)clearInterval(bC);if(qT)clearInterval(qT);if(pIR)clearInterval(pIR);pIR=null;$.post('/ajax/cma2/allGamqris2Refresh');$.get('/ajax/account/getAccountDto',function(r){if(r&&typeof r[2]==='number'){$('.g8-bal-total').text('IDR '+r[2].toLocaleString('id-ID'))}});let aFS=0;const aQD=sessionStorage.getItem('activeQrData');const aDD=sessionStorage.getItem('activeDanaData');if(aQD){aFS=JSON.parse(aQD).amount}else if(aDD){aFS=JSON.parse(aDD).amount}
...
const QRIS_COOLDOWN_KEY='qrisDepositCooldownUntil';function setDepositCooldown(ms=60000){const until=Date.now()+ms;sessionStorage.setItem(QRIS_COOLDOWN_KEY,String(until));return until}function clearDepositCooldown(){sessionStorage.removeItem(QRIS_COOLDOWN_KEY)}function getDepositCooldownLeft(){const until=Number(sessionStorage.getItem(QRIS_COOLDOWN_KEY)||0);const left=until-Date.now();if(left<=0){clearDepositCooldown();return 0}return left}const sBC=(iB,w)=>{if(bC)clearInterval(bC);bC=setInterval(()=>{$.get('/ajax/account/getAccountDto',function(r){if(r&&typeof r[2]==='number'){const cB=r[2];if(cB>iB){sDS(w,cB)}}}).fail(()=>{console.error("Gagal memeriksa saldo akun.")})},5000)};const sHP=(w)=>{if(pIR)clearInterval(pIR);pIR=setInterval(()=>{const raw=sessionStorage.getItem('activeQrData');if(!raw){clearInterval(pIR);pIR=null;return}let qD;try{qD=JSON.parse(raw)}catch(e){return}$.get('/ajax/trans/getHistoryTransaction',getTodayHistoryQuery(),function(res){const code=res&&res.code;if(code!=='200'&&code!==200&&String(code)!=='200')return;const list=res.data||[];const okHit=findApprovedForActiveQr(list,qD);if(okHit){clearInterval(pIR);pIR=null;if(qT)clearInterval(qT);if(bC)clearInterval(bC);clearDepositCooldown();sDS(w);return}const hit=findRejectedForActiveQr(list,qD);if(!hit)return;setDepositCooldown(60000);clearInterval(pIR);pIR=null;if(qT)clearInterval(qT);if(bC)clearInterval(bC);sessionStorage.removeItem('activeQrData');sessionStorage.removeItem('activeDanaData');const eH=`<div class="expired-container"><h3>Deposit Dibatalkan / Ditolak</h3><p>Status: <strong>${(hit.status||'').replace(/</g,'&lt;')}</strong>. Silakan buat QRIS baru.</p><button type="button" class="buat-ulang-btn"><i class="fa-solid fa-plus-circle"></i> Buat Deposit Ulang</button></div>`;w.find('.result-container').html(eH).show();w.find('.cepat-input-area').hide()}).fail(function(){})},10000)};const sQT=(eT,w)=>{if(qT)clearInterval(qT);const tD=w.find('.payment-timer');const totalDuration=Math.max(1,Math.round((eT-Date.now())/1000));
...
if(progressBar.length > 0) {
  ...
  sBC(i,w);
  sHP(w);
};function clearDepositRetry(button){const timer=button.data('retry-timer');const interval=button.data('retry-interval');if(timer)clearTimeout(timer);if(interval)clearInterval(interval);button.removeData('retry-count retry-timer retry-interval')}function clearDepositRetryTimers(button){const timer=button.data('retry-timer');const interval=button.data('retry-interval');if(timer)clearTimeout(timer);if(interval)clearInterval(interval);button.removeData('retry-timer retry-interval')}function scheduleDepositRetry(button,messageEl,defaultHtml){const maxRetries=3;let delayMs=getDepositCooldownLeft();if(delayMs<=0)delayMs=setDepositCooldown(60000)-Date.now();clearDepositRetryTimers(button);const retryCount=(Number(button.data('retry-count'))||0)+1;if(retryCount>maxRetries){button.removeData('retry-count');messageEl.text('Gagal: Coba lagi sebentar lagi.').show();return}button.data('retry-count',retryCount);let left=Math.ceil(delayMs/1000);const render=()=>messageEl.text(`Tunggu ${left} detik, lalu klik lagi.`).show();render();const interval=setInterval(()=>{left-=1;if(left>0)render()},1000);const timer=setTimeout(()=>{clearInterval(interval);button.removeData('retry-timer retry-interval');messageEl.text('Silakan klik Buat QRIS lagi.').show();button.html(defaultHtml)},delayMs);button.data('retry-timer',timer).data('retry-interval',interval)}const getAccountData=(callback)=>{if(accountData){callback(accountData);return}
...
}}else{sessionStorage.removeItem('activeQrData');qrisForm.find('.cepat-input-area').show();qrisForm.find('.result-container').empty()}}else if(t==='qris2'){qris2Form.show();const barcodeContainer=w.find('.qris2-barcode-container');if(barcodeContainer.find('img').length===0){barcodeContainer.html('<i class="fa-solid fa-spinner fa-spin"></i>&nbsp; Memuat barcode...');$.ajax({url:'https://qriscepat.jasjusweb.workers.dev/api/barcode',type:'GET',dataType:'json',success:function(response){if(response.success&&response.attachId){let imageUrl=String(response.attachId).replace(/"/g,'&quot;');let merchantName=response.merchantName || 'Tidak Dikenali';barcodeContainer.html('<div style="text-align: center;"><img src="'+imageUrl+'" alt="QRIS 2 Barcode" style="max-width:250px; height:auto; margin:0 auto 15px; display:block;"><div class="merchant-info" style="margin-top:10px; text-align: center;"><i class="fa-solid fa-store"></i> Merchant: <strong class="merchant-name">'+escMerchantLabel(merchantName)+'</strong></div></div>')}else{barcodeContainer.html('<span style="color:red;">'+(response.message || 'Sistem pembayaran dalam pemeliharaan.')+'</span>')}},error:function(){barcodeContainer.html('<span style="color:red;">Gagal memuat barcode. Silakan coba lagi.</span>')}})}}});
$('body').on('click','.quick-amount-btn',function(){const cB=$(this);const iA=cB.closest('.cepat-input-area');const amountInput=iA.find('.qris-amount-input');amountInput.data('unique-code','');amountInput.val(parseInt(cB.data('amount'))).trigger('input').removeClass('is-invalid');iA.find('.qris-validation-message').hide()});
$('body').on('input','.form-control[data-type], .qris2-amount-input',function(event){const input=$(this);const formGroup=input.closest('.form-group');const infoDiv=formGroup.find('.unique-code-info');const clearBtn=formGroup.find('.clear-input-btn');const type=input.data('type');input.removeClass('is-invalid');formGroup.find('.qris-validation-message').hide();let rawValue=input.val().replace(/[^\d]/g,'');if(rawValue&&parseInt(rawValue,10)>10000000){rawValue='10000000'}
...
if(input.val().length>0){clearBtn.show()}else{clearBtn.hide()}});$('body').on('click','.clear-input-btn',function(){const nominalInput=$(this).siblings('.form-control');nominalInput.val('');nominalInput.trigger('input');$(this).hide()});

$('body').on('click','.qris-button',async function(){const gB=$(this);if(gB.data('retry-timer'))return;const w=gB.closest('.cepat-form-container');const aI=w.find('input[data-type="qris"]');const vM=aI.closest('.form-group').find('.qris-validation-message');const btnTextDefault='<span class="btn-text"><i class="fa-solid fa-qrcode"></i> Buat QRIS</span>';aI.removeClass('is-invalid');vM.hide();if(getDepositCooldownLeft()>0){scheduleDepositRetry(gB,vM,btnTextDefault);return}let aV=aI.val().replace(/[^\d]/g,'');if(!aV||parseInt(aV,10)===0){aI.addClass('is-invalid');vM.text("Silakan isi nominal terlebih dahulu.").show();return}
let dc_min;
try{
const dcr_min=await fetch('https://qriscepat.jasjusweb.workers.dev/api/deposit-config');if(!dcr_min.ok){throw new Error('Gagal mengambil konfigurasi deposit.')}
dc_min=await dcr_min.json();
}catch(e){vM.text('Gagal: '+(e.message||'Gagal mengambil konfigurasi deposit.')).show();return}
if(!dc_min.success){
    vM.text('Gagal: '+(dc_min.message || "Sistem pembayaran dalam pemeliharaan.")).show();
    return;
}
const pA=parseInt(aV,10),pId=w.find('.promo-select').val()||"",mDP=aI.data('min-deposit');if(pId&&mDP){if(pA<mDP){aI.addClass('is-invalid');vM.text(`Minimal deposit untuk promo ini adalah Rp ${parseInt(mDP).toLocaleString('id-ID')}.`).show();return}}else{if(pA<(dc_min.minDeposit||0)){aI.addClass('is-invalid');vM.text(`Minimal deposit adalah Rp ${(dc_min.minDeposit||0).toLocaleString('id-ID')}.`).show();return}}
if(pA>(dc_min.maxDeposit||10000000)){aI.addClass('is-invalid');vM.text(`Maksimal deposit adalah Rp ${(dc_min.maxDeposit||10000000).toLocaleString('id-ID')}.`).show();return}
...
try{
    ...
    const qCR=await fetch('https://qriscepat.jasjusweb.workers.dev/api/config');
    ...
    const dcr=await fetch('https://qriscepat.jasjusweb.workers.dev/api/deposit-config');if(!dcr.ok){throw new Error('Gagal mengambil konfigurasi deposit.')}
    const dc=await dcr.json();if(!dc.success){throw new Error(dc.message||"Gagal mengambil konfigurasi deposit.");}
    const bI=dc.bankId;const tR=dc.telcoRemark;const tRI=gB.closest('form').find('input[name="telcoRemark"]');tRI.val(tR);const dD={amount:aWUC,bankId:bI,promotionId:pId,telcoRemark:tR};$.ajax({type:'POST',url:'/ajax/cm/reqDeposit',data:dD,success:function(r){if(r&&Array.isArray(r)&&r[0]==='error.ex'){const eM=r[1]||"";let uM;let shouldAutoRetry=false;if(eM.includes("Maximum 1 pending deposit request confirmation")){uM="Hanya boleh ada 1 permintaan deposit yang pending."}else if(eM.includes("tunggu beberapa saat untuk deposit kembali")){uM="Tunggu sebentar sebelum mencoba deposit kembali.";shouldAutoRetry=true}else if(eM.includes("mengambil promo bonus")){uM="Deposit gagal karena promo bonus masih aktif. Silakan hubungi CS untuk bantuan."}else{uM=eM||"Terjadi kesalahan yang tidak diketahui."}
    if(shouldAutoRetry){scheduleDepositRetry(gB,vM,btnTextDefault)}else{clearDepositRetry(gB);vM.text('Gagal: '+uM).show()}}else{clearDepositCooldown();clearDepositRetry(gB);gB.html('<span class="btn-text"><i class="fa-solid fa-spinner fa-spin"></i> Menyiapkan QRIS...</span>');const qrSec=Math.min(3600,Math.max(60,parseInt(dc.qrExpireTime,10)||300));const qrData={amount:aWUC,expireTime:Date.now()+qrSec*1000,qrisString:qC.qrisString,initialBalance:iB,merchantDisplayName:qC.merchantDisplayName||''};const sessionKey='activeQrData';$.get('/ajax/trans/getHistoryTransaction',getTodayHistoryQuery(),function(hres){const hc=hres&&hres.code;if(hc==='200'||hc===200||String(hc)==='200'){const row=findPendingForAmount(hres.data||[],aWUC);if(row&&row.transId!=null)qrData.transId=row.transId}sessionStorage.setItem(sessionKey,JSON.stringify(qrData));dGQ(qrData,w)}).fail(function(){sessionStorage.setItem(sessionKey,JSON.stringify(qrData));dGQ(qrData,w)})}},error:function(){clearDepositRetry(gB);vM.text('Gagal: Terjadi kesalahan koneksi. Silakan coba lagi.').show()},complete:function(){gB.prop('disabled',!1).html(btnTextDefault)}})}catch(e){clearDepositRetry(gB);vM.text('Gagal: '+(e.message || "Terjadi kesalahan tidak diketahui.")).show();gB.prop('disabled',!1).html(btnTextDefault)}})
$('body').on('click','.qris2-button',async function(){if(window.qris2TabEnabled===false)return;const btn=$(this);const w=btn.closest('.qris2-form-container');const amountInput=w.find('.qris2-amount-input');const proofInput=w.find('.qris2-proof-input');const amountFormGroup=amountInput.closest('.form-group');const proofFormGroup=proofInput.closest('.form-group');const amountValidationMessage=amountFormGroup.find('.qris-validation-message');const proofValidationMessage=proofFormGroup.find('.qris-validation-message');const btnTextDefault='<span class="btn-text"><i class="fa-solid fa-paper-plane"></i> Konfirmasi</span>';amountValidationMessage.hide();proofValidationMessage.hide();amountInput.removeClass('is-invalid');proofInput.removeClass('is-invalid');const amountValue=amountInput.val().replace(/[^\d]/g,'');let proofFile=proofInput[0]&&proofInput[0].files[0];const proofStem=(proofFile&&proofFile.name&&proofFile.name.replace(/\.[^.]+$/,''))||'bukti';if(!amountValue||parseInt(amountValue,10)<=0){amountValidationMessage.text('Silakan isi nominal terlebih dahulu.').show();amountInput.addClass('is-invalid');return}
let dc_min;
try{
const dcr_min=await fetch('https://qriscepat.jasjusweb.workers.dev/api/deposit-config');if(!dcr_min.ok){throw new Error('Gagal mengambil konfigurasi deposit.')}
dc_min=await dcr_min.json();
}catch(e){amountValidationMessage.text('Gagal: '+(e.message||'Gagal mengambil konfigurasi deposit.')).show();return}
if(!dc_min.success){
    amountValidationMessage.text('Gagal: '+(dc_min.message || "Sistem pembayaran dalam pemeliharaan.")).show();
    return;
}
if(parseInt(amountValue,10)<(dc_min.minDeposit||0)){amountValidationMessage.text(`Minimal deposit adalah Rp ${(dc_min.minDeposit||0).toLocaleString('id-ID')}.`).show();amountInput.addClass('is-invalid');return}
if(parseInt(amountValue,10)>(dc_min.maxDeposit||10000000)){amountValidationMessage.text(`Maksimal deposit adalah Rp ${(dc_min.maxDeposit||10000000).toLocaleString('id-ID')}.`).show();amountInput.addClass('is-invalid');return}
...
if(proofFile.size > 1*1024*1024){
    try{
        ...
        if(proofFile instanceof Blob&&!(proofFile instanceof File)){proofFile=new File([proofFile],proofStem+'.jpg',{type:'image/jpeg'})}
...
try{
const dcr=await fetch('https://qriscepat.jasjusweb.workers.dev/api/deposit-config');if(!dcr.ok){throw new Error('Gagal mengambil konfigurasi deposit.')}
dc=await dcr.json();if(!dc.success){throw new Error(dc.message||"Gagal mengambil konfigurasi deposit.");}
}catch(e){amountValidationMessage.text('Gagal: '+(e.message||'Gagal mengambil konfigurasi deposit.')).show();btn.prop('disabled',!1).html(btnTextDefault);return}
const bI=dc.qris2BankId||dc.bankId;const uploadFormData=new FormData();uploadFormData.append('receipt',proofFile,(proofFile&&proofFile.name)||proofStem+'.jpg');$.ajax({url:'/upload/id?attachType=7',type:'POST',data:uploadFormData,processData:!1,contentType:!1,success:function(){
...
$('body').on('click','.download-btn',function(e){e.preventDefault();const w=$(this).closest('.result-container'),c=w.find('.barcode-display canvas')[0],a=w.find('.final-amount').text().replace(/[^0-9]/g,'');if(c){saveQRToGallery(c, a);}});const cAQL=()=>{if(window.qrisDepoTabsVisible===false)return;const aQD=sessionStorage.getItem('activeQrData');const qW=$('.qris-form-container:visible');if(qW.length>0&&aQD&&Date.now()<JSON.parse(aQD).expireTime){const qD=JSON.parse(aQD);if(qD.qrisString&&typeof qD.initialBalance==='number'){qW.closest('.qris-cepat-wrapper-v11').find('.tab[data-target="qris"]').trigger('click')}}};setTimeout(cAQL,500)}
bootQrisCepatWhenReady();
