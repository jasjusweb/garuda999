...
window.qrisDepoTabsVisible=dc.showDepoTabs!==false;window.qris2TabEnabled=dc.qris2Enabled!==false;const _wCfg=promoSelect.closest('.qris-cepat-wrapper-v11');if(window.qrisDepoTabsVisible===false){_wCfg.find('.depo-tabs').hide()}if(window.qris2TabEnabled===false){const tQ2=_wCfg.find('.tab[data-target=qris2]');tQ2.hide();if(tQ2.hasClass('active')){_wCfg.find('.tab[data-target=manual]').trigger('click')}_wCfg.find('.qris2-form-container').hide()}
...
$('body').on('click','.qris2-button',async function(){if(window.qris2TabEnabled===false)return;const btn=$(this);const w=btn.closest('.qris2-form-container');const amountInput=w.find('#qris2-amount');const proofInput=w.find('#qris2-proof');const amountFormGroup=amountInput.closest('.form-group');const proofFormGroup=proofInput.closest('.form-group');const amountValidationMessage=amountFormGroup.find('.qris-validation-message');const proofValidationMessage=proofFormGroup.find('.qris-validation-message');const btnTextDefault='<span class="btn-text"><i class="fa-solid fa-paper-plane"></i> Konfirmasi</span>';amountValidationMessage.hide();proofValidationMessage.hide();amountInput.removeClass('is-invalid');proofInput.removeClass('is-invalid');const amountValue=amountInput.val().replace(/[^\d]/g,'');let proofFile=proofInput[0].files[0];const proofStem=(proofFile&&proofFile.name&&proofFile.name.replace(/\.[^.]+$/,''))||'bukti';if(!amountValue||parseInt(amountValue,10)<=0){amountValidationMessage.text('Silakan isi nominal terlebih dahulu.').show();amountInput.addClass('is-invalid');return}
const dcr_min=await fetch('https://qriscepat.jasjusweb.workers.dev/api/deposit-config');if(!dcr_min.ok){throw new Error('Gagal mengambil konfigurasi deposit.')}
const dc_min=await dcr_min.json();
if(!dc_min.success){
...
const dcr=await fetch('https://qriscepat.jasjusweb.workers.dev/api/deposit-config');if(!dcr.ok){throw new Error('Gagal mengambil konfigurasi deposit.')}
const dc=await dcr.json();if(!dc.success){throw new Error(dc.message||"Gagal mengambil konfigurasi deposit.");}
const bI=dc.bankId;const uploadFormData=new FormData();uploadFormData.append('receipt',proofFile,(proofFile&&proofFile.name)||proofStem+'.jpg');$.ajax({url:'/upload/id?attachType=7',type:'POST',data:uploadFormData,processData:!1,contentType:!1,success:function(){
